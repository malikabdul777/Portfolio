...
})({"6QxKo":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
...
let prevAsscentColor = localStorage.getItem("asscentColor") === null ? null : localStorage.getItem("asscentColor");
let randAsscent = asscentColorsArray[Math.floor(Math.random() * asscentColorsArray.length)];
let newAsscent = "";
// console.log(asscentColorsArray.findIndex((el) => el == `${randAsscent}`));
(function() {
    let val = null;
    if (randAsscent !== prevAsscentColor) val = randAsscent;
    else {
        let currInd = asscentColorsArray.findIndex((el)=>el == `${randAsscent}`
        );
        // console.log(currInd);
        if (asscentColorsArray[currInd + 1] != undefined) val = asscentColorsArray[currInd + 1];
        else val = asscentColorsArray[currInd - 1];
    }
    newAsscent = val;
})();
if (prevAsscentColor == newAsscent) console.log("caught");
...
if (localStorage.getItem("shouldReloadSmall") == null) {
    if (window.innerWidth > 450) localStorage.setItem("shouldReloadSmall", true);
}
let shouldReloadLarge = null;
if (localStorage.getItem("shouldReloadLarge") == null) {
    if (window.innerWidth < 450) localStorage.setItem("shouldReloadLarge", true);
}
window.onresize = function(e) {
    ...
    const timer = setInterval(()=>{
        ...
        if (prevTimeStamp == currTimeStamp) {
            if (e.srcElement.innerWidth > 451 && JSON.parse(localStorage.getItem("shouldReloadLarge"))) {
                localStorage.setItem("shouldReloadSmall", true);
                localStorage.setItem("shouldReloadLarge", false);
                location.reload();
            }
            if (e.srcElement.innerWidth <= 450 && JSON.parse(localStorage.getItem("shouldReloadSmall"))) {
...
document.addEventListener("mousemove", (e)=>{
    ...
    cursor.setAttribute("style", `top: ${e.pageY + 8.8}px; left: ${e.pageX + 8.8}px`);
// cursorCircle.setAttribute("style", `top: ${e.pageY}px; left: ${e.pageX}px`);
});
...
burger.addEventListener("click", ()=>{
    ...
    link.forEach((el)=>{
        el.addEventListener("mouseover", ()=>{
            ...
            if (mob.matches) enlargeCursor("cursorGrowSmall");
        });
        el.addEventListener("mouseleave", ()=>{
            mob = window.matchMedia("(max-width: 900px)");
            setTimeout(()=>{
                if (mob.matches) shrinkCursor("cursorGrowSmall");
            }, 1000);
        });
    });
    span.forEach((el)=>{
...
if (!mob.matches) gsap.to(".cont-aboutMe", {
    y: "38%",
    duration: 0.1,
    ease: "power3.out",
    scrollTrigger: {
...
tilesTl.from(".photoshopImg", {
    y: "58%",
    x: "-39%",
    ease: "power3.out"
}, 0);
tilesTl.from(".htmlImg", {
    y: "-48%",
    //  x: "-39%",
    ease: "power3.out"
}, 0);
tilesTl.from(".javascriptImg", {
    scale: 0.6,
    // opacity: 0.6,
    //  x: "-39%",
    ease: "power3.out"
}, 0);
tilesTl.from(".reactImg", {
    y: "39%",
    ease: "power3.out"
}, 0);
tilesTl.from(".scssImg", {
    x: "39%",
    y: "-39%",
    ease: "power3.out"
}, 0);
tilesTl.from(".illuImg", {
    x: "39%",
    y: "39%",
    ease: "power3.out"
}, 0);
tilesTl.from(".pythonImg", {
    x: "49%",
    // y: "39%",
    ease: "power3.out"
}, 0);
...
},{"locomotive-scroll":"aJY2w","@parcel/transformer-js/src/esmodule-helpers.js":"ecToR"}],"aJY2w":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"ecToR"}],"ecToR":[function(require,module,exports) {
...
},{}]},["6QxKo","8fvzf"], "8fvzf", "parcelRequiree3ad")
